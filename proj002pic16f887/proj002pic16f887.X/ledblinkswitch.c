/*
 >>>>> File Name: ledblinkswitch.c
 >>>>> Aim: A C program to Interface an LED and switch.
 * Glow LED as long as the switch is closed.
 * 
 */
...
void main(void) 
{
    ...
    while(1)
    {
        if(RA1==1)  //If there is voltage at RA1
            RA0=1;  //Turn on LED.
        else
            RA0=0;  //Turn off LED.
    }
...
void init()
{
    ...
    TRISA1=1;  //To make the RA1 pin an input pin. RA1 pin used for switch.
    ANS0=0;  //To make AN0 a digital I/O pin; not analog pin.
    ANS1=0;  //To make AN1 a digital I/O pin; not analog pin.
}
//----------------------------------------------------------------------------
/*
 ...
 * 
 >>>>> When there is high voltage(nearly 5V) through switch to the 
 * RA1 pin the LED glows. Therefore RA1 is made as digital input
 * line and RA0 (supplies energy to the LED) is made as digital 
 * output.
 */