/*
 >>>>> File Name: adcmodulevolt.c
 >>>>> Aim: To demonstrate A/D Conversion using internal ADC module of PIC16F887 
 * output the ADC value with corresponding voltage value.
 * 
 */
...
void main(void) 
{
    unsigned char value[11]={"ADC Value: "};  //Each char 8-bit
    unsigned char voltvalue[10]={"Volt val: "};
    unsigned long adcval,voltval;  //32-bit
    ...
    for(int i=0;i<10;i++)  //Display string in variable value.
        data(voltvalue[i]);
    ...
    while(1)
    {
        ...
        command(0x8b);  //Cursor at first line eleventh position
        data((adcval/1000)+48);
        ...
        command(0xca);  //Cursor at second line tenth position
